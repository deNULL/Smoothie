 ...
 */
 ...
 (function(Webvs) {
  ...
  Webvs.Resources = {
    ...
    "tug_ti_texer2.bmp": "data:image/png;base64,",
  };
     ...
     start: function() {
      ...
      var drawFrame = function() {
        if(_this.analyser.isPlaying()) {
          _this.onBeforeUpdate && _this.onBeforeUpdate();
          _this.rootComponent.update();
          _this.onAfterUpdate && _this.onAfterUpdate();
        }
 ...
 Webvs.CodeInstance = Webvs.defineClass(CodeInstance, Object, {
     ...
     */
     getosc: function(band, width, channel) {
      ...
      var pos = Math.max(Math.floor((band - width/2)*(osc.length-1)), 0);
      var end = Math.min(Math.floor((band + width/2)*(osc.length-1)), osc.length - 1);
    ...
    /**
     * avs expression getspec function
     * @memberof Webvs.CodeInstance#
     */
     getspec: function(band, width, channel) {
      var spec = this._analyser.getSpectrum();
      var pos = Math.max(Math.floor((band - width/2)*(spec.length-1)), 0);
      var end = Math.min(Math.floor((band + width/2)*(spec.length-1)), spec.length - 1);
     ...
     */
     setup: function(main, parent) {
      ...
      this._megabuf = main.megabuf;
      this._gmegabuf = main.gmegabuf;
     ...
     generateJs: function(jsFuncs) {
      ...
      _generateGlsl: function(ast, preCompute) {
        ...
        if(ast instanceof Webvs.AstFuncCall) {
          ...
          switch(ast.funcName) {
            case "above":
              return ["(", this._generateGlsl(ast.args[0], preCompute), ">", this._generateGlsl(ast.args[1], preCompute), "?1.0:0.0)"].join("");
            case "below":
              return ["(", this._generateGlsl(ast.args[0], preCompute), "<", this._generateGlsl(ast.args[1], preCompute), "?1.0:0.0)"].join("");
            case "equal":
              return ["(", this._generateGlsl(ast.args[0], preCompute), "==", this._generateGlsl(ast.args[1], preCompute), "?1.0:0.0)"].join("");
            case "if":
              return ["(", this._generateGlsl(ast.args[0], preCompute), "!=0.0?",
                this._generateGlsl(ast.args[1], preCompute), ":",
                this._generateGlsl(ast.args[2], preCompute), ")"].join("");
            case "select":
              ...
              var generateSelect = function(args, i) {
                if(args.length == 1) {
                  ...
                  return ["(("+selectExpr+" === "+i+")?", "("+that._generateGlsl(args[0], preCompute)+"):", "("+generateSelect(_.last(args, args.length-1), i+1)+"))"].join("");
                }
              };
      ...
      _generateJs: function(ast, assign) {
        var prefix;
        ...
        if(ast instanceof Webvs.AstFuncCall) {
          ...
          switch(ast.funcName) {
            ...
            case "assign":
              if (ast.args[0] instanceof Webvs.AstFuncCall) {
                if (ast.args[0].funcName == "if") {
                  return "("+this._generateJs(ast.args[0].args[0])+"!==0?("+
                    this._generateJs(ast.args[0].args[1], true)+"=("+this._generateJs(ast.args[1])+")):("+
                    this._generateJs(ast.args[0].args[2], true)+"=("+this._generateJs(ast.args[1])+")))";
                }
              }
              return "("+this._generateJs(ast.args[0], true)+"=("+this._generateJs(ast.args[1])+"))";
            case "megabuf":
              return assign ? ("this._megabuf[" + this._generateJs(ast.args[0]) + "]") :
                ("(this._megabuf[" + this._generateJs(ast.args[0]) + "]||0)");
            case "gmegabuf":
              return assign ? ("this._gmegabuf[" + this._generateJs(ast.args[0]) + "]") :
                ("(this._gmegabuf[" + this._generateJs(ast.args[0]) + "]||0)");
            default:
        ...
        if(ast instanceof Webvs.AstAssignment) {
          return this._generateJs(ast.lhs, true) + "=" + this._generateJs(ast.expr);
        }
        if(ast instanceof Webvs.AstProgram) {
 ...
 */
 function Colorfade(options) {
  ...
  Colorfade.super.constructor.apply(this, arguments);
}
Webvs.Colorfade = Webvs.defineClass(Colorfade, Webvs.Component, {
  componentName: "Colorfade",

    /**
     * initializes the Colorfade component
     * @memberof Webvs.Colorfade#
     */
     init: function(gl, main, parent) {
      Colorfade.super.init.call(this, gl, main, parent);
      //this.program.init(gl);
    },
     ...
     */
     destroy: function() {
      Colorfade.super.destroy.call(this);
      //this.program.cleanup();
    }
  });
...
function ColorMapProgram(key, blendMode) {
  ...
  switch(key) {
    ...
    default: throw new Error("Unknown colormap key function " + key);
  }
 ...
 */
 function DynamicMovement(options) {
  ...
  this.code = codeGen.generateJs(["init", "onBeat", "perFrame"]);
  var glslCode = codeGen.generateGlsl(["perPixel"], ["x", "x1", "y", "d", "r"], this.code);
...
Webvs.TexerProgram = Webvs.defineClass(TexerProgram, Webvs.ShaderProgram, {
  draw: function(vertices, texVertices, indices, colors, image) {
    ...
    if(colors) {
      ...
      this.setVertexAttribArray("a_color", new Float32Array(vertices.length * 3 / 2), 3);
    }
